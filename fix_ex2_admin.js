...
// 1. Update Server.js (Add Delete API)
const serverPath = 'exercise-2/backend/server.js';
if (fs.existsSync(serverPath)) {
    let content = fs.readFileSync(serverPath, 'utf8');
    
    if (!content.includes('/api/admin/delete-user')) {
        const deleteApi = `
...
`;
        content = content.replace('// Health check', deleteApi + '\n// Health check');
        fs.writeFileSync(serverPath, content, 'utf8');
    }
}

// 2. Update Exercise-2 Frontend Admin Logic
const ex2AppPath = 'exercise-2/frontend/app.js';
if (fs.existsSync(ex2AppPath)) {
    let content = fs.readFileSync(ex2AppPath, 'utf8');
    
    // Replace renderAdmin, editUserPoints, deleteUser
...
\t});
    ...
    // Regex to match from function renderAdmin() to the end of deleteUser(email)
    content = content.replace(/function renderAdmin\(\)\{[\s\S]*?function deleteUser\(email\) \{[\s\S]*?\}\s*(?=function loadFromStorage)/, newAdminLogic + '\n\n');
